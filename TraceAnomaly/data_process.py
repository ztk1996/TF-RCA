...
def statistic_unique_trace_length(trace_data):
    ...
    for trace_id, trace in tqdm(trace_data.items()):
        ...
        unique_service_sequence = get_unique_service_sequence(
            trace['service_seq'])
        ...
        for i in range(1, trace_length):
            if base_service_seq[i] != trace['service_seq'][i]:
                ...
                different_time[trace['service_seq'][i]].append(
                    trace['time_seq'][i-1])
        new_trace = {}
...
if __name__ == '__main__':
    ...
    test_normal_trace_data = get_api_seq_and_time_seq(
        root + 'data_normal.json')
    _, test_normal_trace_data = statistic_unique_trace_length(
        test_normal_trace_data)
    # 1.换成单故障数据集 '/home/kagaya/work/TF-RCA/data/preprocessed/trainticket/2022-05-06_17-28-43/data.json' 的异常数据； 2.换成变更数据集 '/home/kagaya/work/TF-RCA/data/preprocessed/trainticket/2022-05-11_00-06-32/data.json' 的异常部分
    test_abnormal_trace_data = get_api_seq_and_time_seq(
        root + 'data_abnormal.json')
    _, test_abnormal_trace_data = statistic_unique_trace_length(
        test_abnormal_trace_data)
    print(len(train_trace_data), len(test_normal_trace_data),
          len(test_abnormal_trace_data))
    # merge 3 dict
    trace_data = {}
    ...
    data_process_for_trace_anomaly(
        test_normal_trace_data, seq_set, 'test_normal')
    data_process_for_trace_anomaly(
        test_abnormal_trace_data, seq_set, 'test_abnormal')
